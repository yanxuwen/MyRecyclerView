...
import com.yanxuwen.PinyinHelper.Pinyin;
...
public class IndexBarDataHelperImpl implements IIndexBarDataHelper {
    ...
    public IIndexBarDataHelper convert(List<? extends BaseIndexPinyinBean> datas) {
        ...
        for (int i = 0; i < size; i++) {
            ...
            if (indexPinyinBean.isNeedToPinyin()) {
                ...
                String target = indexPinyinBean.getTarget();//取出标题
                pySb.append(target);
            }
        }
        return this;
        ...
        for (int i = 0; i < size; i++) {
            ...
            if (indexPinyinBean.isNeedToPinyin()) {
                ...
                if (tagString.matches("[A-Z]")) {//如果是A-Z字母开头
                    ...
                    indexPinyinBean.setSuspensionTag(tagString);
                } else {//特殊字母这里统一用#处理
                    indexPinyinBean.setBaseIndexTag("#");
                    indexPinyinBean.setSuspensionTag("#");
                }
            }else{
                String tagString=indexPinyinBean.getTarget();
                String indexString=indexPinyinBean.getIndexString();
                if(indexString!=null) {//先判断是否有特殊的索引值，没有则取tag
                    indexPinyinBean.setBaseIndexTag(indexString);
                    indexPinyinBean.setSuspensionTag(tagString);
                }else{
                    indexPinyinBean.setBaseIndexTag(tagString);
                    indexPinyinBean.setSuspensionTag(tagString);
    ...
    public IIndexBarDataHelper getSortedIndexDatas(List<? extends BaseIndexPinyinBean> sourceDatas, List<String> indexDatas) {
        ...
        for (int i = 0; i < size; i++) {
            ...
            String indexString=sourceDatas.get(i).getIndexString();
            if(indexString!=null){//先判断是否有特殊的索引值，没有则取tag
                if (!indexDatas.contains(indexString)) {//则判断是否已经将这个索引添加进去，若没有则添加
                    indexDatas.add(indexString);
                }
            }else{
